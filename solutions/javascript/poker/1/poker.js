...
class Hand {
  constructor(hand_str) {
    ...
    const cards = hand_str.split(" ").map(getCard).sort(cardSort); // sort cards by value
    ...
    cards.forEach((card, idx) => {
      this.value += card.value * Math.pow(BASE, idx); // firstly, each card has a value
    ...
    if (!straight) {
      if (
        ...
        const ace = cards.pop();
        ace.value = 0; // represents a 1, which has no value,
        cards.unshift(ace);
        straight = true;
      }
    }
    ...
    if (values.size === 2) {
      ...
      for (const [value, occurrences] of values.entries()) {
        if (occurrences === 4) {
          // FOUR_OF_A_KIND;
          this.value += value * Math.pow(BASE, orders.FOUR_OF_A_KIND);
          break;
        } else if (occurrences === 3) {
          // FULL_HOUSE_THREE
          this.value += value * Math.pow(BASE, orders.FULL_HOUSE_THREE);
        } else if (occurrences === 2) {
          // FULL_HOUSE_PAIR
          this.value += value * Math.pow(BASE, orders.FULL_HOUSE_PAIR);
        }
      }
      ...
      for (const [value, occurrences] of values.entries()) {
        if (occurrences === 3) {
          // THREE_OF_A_KIND
          this.value += value * Math.pow(BASE, orders.THREE_OF_A_KIND);
          break;
        } else if (occurrences === 2) {
          pairs.push(value);
        }
      ...
      if (pairs.length) {
        ...
        this.value += pairs[0] * Math.pow(BASE, orders.PAIR_LOW);
        this.value += pairs[1] * Math.pow(BASE, orders.PAIR_HIGH);
      }
    } else if (values.size === 4) {
      // single pair
      for (const [value, occurrences] of values.entries()) {
        if (occurrences === 2) {
          this.value += value * Math.pow(BASE, orders.PAIR_LOW);
          break;
        }
      }
    } else {
      ...
      if (straight) {
        if (flush) {
          // STRAIGHT_FLUSH
          this.value += cards[4].value * Math.pow(BASE, orders.STRAIGHT_FLUSH);
        } else {
          // STRAIGHT
          this.value += cards[4].value * Math.pow(BASE, orders.STRAIGHT);
        }
      } else if (flush) {
        // FLUSH
...
export const bestHands = (hands) => {
  ...
  const sortedHands = hands.sort((h1, h2) => h2.value - h1.value);
